...
export default function AdminBilling() {
  ...
  const hostedCheckoutUrl =
    import.meta.env.VITE_PAGARME_PAYMENT_LINK ||
    "https://payment-link-v3.pagar.me/pl_eAvMyKzQlDxV2xPUyha25LZ16nJWOqop";
  ...
  const handleCheckout = async () => {
    ...
    try {
      toast({
        ...
        description: "Você será levado ao checkout seguro do Pagar.me.",
      });
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Plano atual e período de acesso da sua academia.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <Badge
              variant={subscription?.isPastDue ? "destructive" : "secondary"}
            >
              {statusLabel}
            </Badge>
            {subscription?.isTrial && (
              <p className="text-sm text-muted-foreground">
                Restam {subscription.daysRemainingTrial ?? 0} dia(s) no período
                de teste.
              </p>
            )}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Depois de escolher, finalize no checkout para liberar acesso
              contínuo.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="rounded-md border border-dashed p-3 text-sm text-muted-foreground">
              O pagamento será feito no checkout seguro do Pagar.me após clicar
              no botão abaixo.
            </div>

            <Button
              onClick={handleCheckout}
              disabled={submitting || !selectedPlan || !profile?.academy_id}
            >
              {submitting ? (
              ...
              {selectedPlan
                ? `Pagar ${formatCurrency(selectedPlan.price)} e ativar PRO`
                : "Selecionar plano para continuar"}
            </Button>
          </CardContent>
        </Card>
      </div>
